...
The primary empirical artifact of this research is the \textbf{NepaliNutriDB} dataset. Following comprehensive data quality auditing and laboratory-backed ingredient calibration from USDA FoodData Central, the operational database comprises \textbf{129 verified foods} (124 authentic Nepali culinary preparations and 5 ubiquitous dietary staples).
...
\midrule
\textbf{Total Database Size} & & \textbf{129} & \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\textbf{Classification Accuracy} ($\tau = 0.5$) & \textbf{90.00\%} & 86.67\% \\
\textbf{Precision} & \textbf{100.00\%} & 100.00\% \\
\textbf{Recall} & \textbf{75.00\%} & 66.67\% \\
\textbf{F1-Score} & \textbf{0.8571} & 0.8000 \\
\midrule
\textbf{Mean Absolute Error (MAE)} & \textbf{0.0274} & 0.0358 \\
\textbf{Root Mean Squared Error (RMSE)} & \textbf{0.0412} & 0.0536 \\
\textbf{Coefficient of Determination ($R^2$)} & \textbf{0.8997} & 0.8302 \\
\midrule
\textbf{Score Spread Range} & 0.061 --- 0.901 & 0.070 --- 0.872 \\
\textbf{Score Standard Deviation ($\sigma$)} & 0.141 & 0.133 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  \item \textbf{Superior Explanatory Power ($R^2 = 0.8997$):} XGBoost captured 89.97\% of the nutritional variance, significantly outperforming Random Forest ($R^2 = 0.8302$).
  \item \textbf{Minimal Error (MAE = 0.0274):} On a 0.0 to 1.0 scoring scale, the mean absolute error of XGBoost is only 2.74 percentage points, proving exceptional calibration.
  \item \textbf{Perfect Precision (100.00\%):} In identifying nutritionally superior foods against low-quality options, XGBoost achieved 100\% precision ($\text{F1} = 0.8571$), guaranteeing zero false-positive junk food recommendations.
\end{itemize}
