...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Data Engineering Pipeline \& NepaliNutriDB:} Construction of an authoritative 129-food database mathematically synthesized via a recipe deconstruction pipeline. Raw ingredient profiles are extracted from USDA FoodData Central (Foundation Foods \& SR Legacy), eliminating arbitrary manual estimation and tagging each derived item with verifiable data provenance (\texttt{Calculated\_from\_USDA}).
  \item \textbf{Deterministic Safety Layer:} Strict clinical rule filtering enforcing dietary restrictions (vegetarian/vegan), allergen safety, and disease-specific macro thresholds (e.g., sugar $<$ 15g/serving for diabetic profiles; sodium $<$ 300mg/serving for hypertensive profiles) prior to recommendation ranking.
  \item \textbf{Machine Learning Recommendation Engine:} Implementation and comparative empirical evaluation of an \textbf{XGBoost Regressor} alongside a Random Forest Regressor. Trained on a normalized nutritional density objective function with synthetic boundary anchors, the XGBoost model achieved \textbf{90.00\% classification accuracy}, \textbf{100\% precision}, an \textbf{MAE of 0.0274}, an \textbf{RMSE of 0.0412}, and an \textbf{$R^2$ score of 0.8997}.
...
\begin{tikzpicture}[node distance=0.8cm and 1.2cm, auto]
  ...
  \node (db)         [process, below=of dbsafety]       {NepaliNutriDB (129 Foods)\\(SQLite / Relational DB)};
...
For each basic ingredient $I_i$, authoritative nutritional baselines per 100\,g are retrieved from the \textbf{USDA FoodData Central} laboratory database. The total nutrient vector of the dish is computed through linear aggregation:
\begin{equation}
  \mathbf{N}_{\text{total}}(D) = \sum_{i=1}^n \left( \frac{w_i}{100} \times \mathbf{N}(I_i) \right)
\end{equation}
...
Computed via the \textbf{Mifflin-St Jeor Equation}:
\begin{align}
...
Scaled by activity coefficient $\alpha \in \{1.20, 1.375, 1.55, 1.725, 1.90\}$:
\begin{equation}
...
The primary empirical artifact of this research is the \textbf{NepaliNutriDB} dataset. To ensure complete academic defensibility and eliminate speculative approximations, all composite Nepali dishes were transitioned to a \textbf{100\% mathematically derived recipe aggregation architecture}. 

Rather than relying on unverified estimations, composite foods are computed as deterministic linear combinations of verified raw agricultural building blocks cataloged in \textbf{USDA FoodData Central Foundation Foods} [8] and the \textbf{Food Composition Table for Nepal} published by the Government of Nepal (DFTQC / FAO) [7]:

\begin{equation}
\text{Nutrient}_{\text{dish}} = \sum_{i=1}^{k} \left( \text{Nutrient}_{i,\text{per 100g}} \times \frac{w_i}{100} \right)
\label{eq:recipe_sum}
\end{equation}

where $w_i$ represents the gram weight of ingredient $i$ in a standard culinary preparation, and $\sum_{i=1}^{k} w_i = \text{serving\_size\_g}$. 

The operational database comprises \textbf{129 verified foods} (124 authentic Nepali culinary preparations and 5 international dietary staples):
\begin{itemize}
  \item \textbf{64 Composite Cooked Dishes (\texttt{USDA\_Recipe\_Calc}):} Mathematically aggregated from verified raw ingredients (e.g., Momos, Dal Bhat, Thukpa, Chow Mein, Curries, Rotis, Tarkaris).
  \item \textbf{35 Foundation Commodities (\texttt{USDA}):} Direct single-ingredient reference foods (e.g., eggs, whole milk, chicken breast, oats, apples).
  \item \textbf{25 Indigenous Staples (\texttt{FAO\_Nepal\_2012}):} Authentic native Himalayan crops and preparations (e.g., Dhido, Kodo, Fapar, Gundruk, Kwati, Gahat ko Dal).
  \item \textbf{5 Traditional Grain Preparations (\texttt{NARC}):} Documented by the Nepal Agricultural Research Council (e.g., Chiura, Baji, Juju Dhau).
\end{itemize}
...
\centering
\small
\caption{Nutritional Category Breakdown in NepaliNutriDB (129 Verified Foods)}
\label{tab:categories}
\begin{tabular}{llcc}
\toprule
\textbf{Category} & \textbf{Representative Food Items} & \textbf{Count} & \textbf{Primary Source} \\
\midrule
Nepali Snacks (\texttt{nepali\_snack}) & Buff Momo, Chicken Momo, Chatamari, Bara, Sel Roti & 24 & USDA Recipe / FAO Nepal \\
Nepali Staples (\texttt{nepali\_staple}) & Dal Bhat, Dhido, Chiura, Khichdi, Masoor Dal & 17 & USDA / FAO Nepal / NARC \\
Nepali Meats (\texttt{nepali\_meat}) & Sekuwa, Choila, Sukuti, Buff Curry, Chicken Curry & 13 & USDA Recipe Calc \\
Nepali Vegetables (\texttt{nepali\_vegetable}) & Rayo Saag, Cauliflower Tarkari, Mula ko Tarkari & 11 & USDA Recipe / FAO Nepal \\
Nepali Sweets (\texttt{nepali\_sweet}) & Yomari, Lakhamari, Jeri, Lal Mohan, Kheer, Juju Dhau & 10 & USDA Recipe / NARC \\
Nepali Curries (\texttt{nepali\_curry}) & Aloo Tama, Gundruk ko Jhol, Aloo Gobi, Paneer Curry & 9 & USDA Recipe / FAO / NARC \\
Nepali Beverages (\texttt{nepali\_beverage}) & Chiya, Masala Tea, Lassi, Chhyang, Mohi, Tongba & 8 & FAO Nepal / USDA Recipe \\
Nepali Pickles (\texttt{nepali\_pickle}) & Tomato Achar, Mula Achar, Til ko Achar, Gundruk Achar & 7 & USDA Recipe / FAO Nepal \\
Nepali Breads (\texttt{nepali\_bread}) & Puri, Roti, Tornak, Naan, Kodo ko Roti, Fapar ko Roti & 6 & USDA Recipe / FAO Nepal \\
Proteins (\texttt{protein}) & Boiled Egg, Chicken Breast, Buff Meat Lean, Tofu & 4 & USDA FoodData Central \\
International Foods (\texttt{international}) & Margherita Pizza, Chicken Burger, French Fries, Toast & 4 & USDA FoodData Central \\
Dairy (\texttt{dairy}) & Paneer Fresh, Curd / Plain Dahi, Whole Cow Milk & 3 & USDA FoodData Central \\
Grains (\texttt{grain}) & White Rice Cooked, Brown Rice Cooked, Oatmeal & 3 & USDA FoodData Central \\
Vegetables \& Aromatics (\texttt{vegetable}) & Garlic (Raw), Ginger (Raw), Chopped Onion & 3 & USDA FoodData Central \\
Fruits (\texttt{fruit}) & Mustang Apple, Malbhog Banana, Suntala Orange & 3 & USDA FoodData Central \\
Healthy Fats \& Oils (\texttt{fat\_oil}) & Mustard Oil (14g serving), Ghee (14g serving) & 2 & USDA FoodData Central \\
Beverages (\texttt{beverage}) & Ilam Green Tea & 1 & USDA FoodData Central \\
Nuts \& Healthy Snacks (\texttt{snack}) & Jumla Okhar (Walnuts, 28g serving) & 1 & USDA FoodData Central \\
\midrule
\textbf{Total Database Size} & & \textbf{129} & \textbf{Zero Manual Estimations} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Food Log Interface:} Enables rapid search across 129 items, custom portion selection in grams, and meal-type grouping (Breakfast, Lunch, Dinner, Snack) with delete actions.
...
The recommendation engine was subjected to rigorous empirical evaluation. The dataset of 129 real foods was augmented with 17 synthetic extreme anchors, yielding 146 total training samples. The dataset was partitioned into an 80\% training set (116 samples) and a 20\% test set (30 samples) using stratified random splitting.
...
\begin{itemize}
  \item \textbf{XGBoost:} $n\_estimators=100$, $max\_depth=4$, $learning\_rate=0.10$, $subsample=0.8$, $colsample\_bytree=0.8$, $random\_state=42$.
  \item \textbf{Random Forest:} $n\_estimators=100$, $max\_depth=8$, $min\_samples\_leaf=2$, $random\_state=42$.
\end{itemize}
...
\midrule
\textbf{Classification Accuracy} ($\tau = 0.5$) & \textbf{93.33\%} & 90.00\% \\
\textbf{Precision} & \textbf{100.00\%} & 100.00\% \\
\textbf{Recall} & \textbf{84.62\%} & 76.92\% \\
\textbf{F1-Score} & \textbf{0.9167} & 0.8696 \\
\midrule
\textbf{Mean Absolute Error (MAE)} & \textbf{0.0317} & 0.0386 \\
\textbf{Root Mean Squared Error (RMSE)} & \textbf{0.0396} & 0.0500 \\
\textbf{Coefficient of Determination ($R^2$)} & \textbf{0.9116} & 0.8592 \\
\midrule
\textbf{Score Spread Range} & 0.064 --- 0.899 & 0.082 --- 0.862 \\
\textbf{Score Standard Deviation ($\sigma$)} & 0.150 & 0.140 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  \item \textbf{Superior Explanatory Power ($R^2 = 0.9116$):} XGBoost captured 91.16\% of the nutritional variance on the mathematically verified dataset, significantly outperforming Random Forest ($R^2 = 0.8592$).
  \item \textbf{Minimal Error (MAE = 0.0317):} On a 0.0 to 1.0 scoring scale, the mean absolute error of XGBoost is only 3.17 percentage points, proving exceptional calibration.
  \item \textbf{Flawless Precision (100.00\%) \& High F1 (0.9167):} In identifying nutritionally superior foods against low-quality options, XGBoost achieved 100\% precision with 84.62\% recall, guaranteeing zero false-positive junk food recommendations.
\end{itemize}
